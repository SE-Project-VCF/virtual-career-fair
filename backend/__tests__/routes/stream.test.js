...
const app = createTestApp(streamRouter);
...
describe("GET /api/stream-unread", () => {
  ...
  it("calculates unread correctly", async () => {
    ...
    mockQueryChannels.mockResolvedValue([
      {
        state: {
          read: { "test-uid": { last_read: lastRead } },
          messages: [
            { created_at: new Date("2024-01-02"), user: { id: "other-user" } },
            { created_at: new Date("2024-01-02"), user: { id: "test-uid" } },
            { created_at: new Date("2023-12-31"), user: { id: "other-user" } },
          ],
        },
      },
    ]);
  ...
  it("skips channel when state is missing", async () => {
    mockQueryChannels.mockResolvedValue([{ state: null }, { state: undefined }]);
    const res = await request(app)
      .get("/api/stream-unread")
      .set("Authorization", authHeader());
    expect(res.status).toBe(200);
  ...
  it("skips channel when lastRead or messages missing", async () => {
    mockQueryChannels.mockResolvedValue([
      { state: { read: {}, messages: [] } },
